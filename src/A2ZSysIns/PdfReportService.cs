...
namespace A2ZSysIns
{
    internal static class PdfReportService
    {
        public static async Task<string> SavePdfAsync(InspectionReport report, Window owner)
        {
            ...
            await Task.Run(() => BuildPdf(report, path));
        ...
        private static void BuildPdf(InspectionReport r, string path)
        {
            ...
            foreach (var s in r.Scores)
                writer.ResultLine(Safe(s.Category), Safe(s.Status), Safe(s.Reason));
            ...
            if (unavailable.Count == 0 && r.Limitations.Count == 0)
                writer.Bullet("No collector explicitly reported an unavailable measurement. This still does not guarantee future reliability.");
            else
            {
            ...
            {
                writer.ResultLine(Safe(d.Model), Safe(d.Assessment),
                    "Capacity " + Collectors.FormatBytes(d.SizeBytes) + "; life indicator " +
                    (d.RemainingLifePercent.HasValue ? d.RemainingLifePercent.Value.ToString("0") + "%" : "Cannot measure") +
                    "; SMART " + Safe(d.SmartStatus));
            }
            ...
            foreach (var s in r.Sensors.Where(EvidenceEngine.ActualTemperature))
                writer.ResultLine(Safe(s.Hardware) + " / " + Safe(s.Name), Number(s.Current, s.Unit),
                    "min " + Number(s.Minimum, s.Unit) + "; max " + Number(s.Maximum, s.Unit));

            writer.Heading("Windows event evidence");
            foreach (var e in r.Events)
                writer.ResultLine(Safe(e.Source) + " / " + e.EventId, e.Count + " occurrence(s)", Safe(e.Cause));

            writer.Heading("Measurement coverage");
            foreach (var m in r.Measurements)
                writer.ResultLine(Safe(m.Target), Safe(m.Status), Safe(m.Source) + " - " + Safe(m.Reason));
        ...
        private sealed class PdfWriter
        {
            ...
            public void Finding(Finding f)
            {
                ...
                Ensure(h + 3);
                var fill = string.Equals(f.Severity, "Critical", StringComparison.OrdinalIgnoreCase) ? XColor.FromArgb(255, 231, 231) :
                           string.Equals(f.Severity, "Attention", StringComparison.OrdinalIgnoreCase) ? XColor.FromArgb(255, 247, 220) : XColor.FromArgb(234, 240, 246);
                _gfx.DrawRectangle(new XSolidBrush(fill), Left, _y, Width, h);
                var yy = _y + 16;
            ...
            public void FooterAllPages(string text)
            {
                for (var i = 0; i < _doc.Pages.Count; i++)
                {
                    using (var g = XGraphics.FromPdfPage(_doc.Pages[i], XGraphicsPdfPageOptions.Append))
                    {
                        ...
                        g.DrawString(text + "  |  Page " + (i + 1) + " of " + _doc.Pages.Count, _small, _muted,
                            new XRect(Left, _doc.Pages[i].Height.Point - 26, _doc.Pages[i].Width.Point - Left - Right, 14), XStringFormats.TopLeft);
                    }
                }
            }
