    ...
    internal static class DarkReportPreviewService
    {
        ...
        public static FlowDocument Build(InspectionReport r)
        {
            ...
            foreach (var f in r.Findings)
            {
                ...
                p.Inlines.Add(new Run(Safe(f.Recommendation) + "\n")); p.Inlines.Add(new Run("Confidence: " + Safe(f.Confidence) + " | Priority: " + Safe(f.ActionLevel)) { Foreground = Muted, FontSize = 9 }); d.Blocks.Add(p);
            }
            ...
            Heading(d, "Measurement coverage"); foreach (var x in r.Measurements) AddEvidence(d, Safe(x.Target), Safe(x.Status), Safe(x.Source) + " - " + Safe(x.Reason));
        ...
        private static void AddKey(FlowDocument d, string key, string value) { var p = new Paragraph { Margin = new Thickness(0, 1, 0, 3) }; p.Inlines.Add(new Run(Safe(key) + ": ") { FontWeight = FontWeights.Bold, Foreground = Accent }); p.Inlines.Add(new Run(Safe(value)) { Foreground = Text }); d.Blocks.Add(p); }
        private static void AddEvidence(FlowDocument d, string category, string status, string reason) { var p = new Paragraph { Margin = new Thickness(0, 2, 0, 4), Padding = new Thickness(6), Background = Surface, BorderBrush = Line, BorderThickness = new Thickness(0, 0, 0, 1) }; p.Inlines.Add(new Run(Safe(category)) { FontWeight = FontWeights.Bold, Foreground = Text }); p.Inlines.Add(new Run("  [" + Safe(status) + "]  ") { Foreground = Accent, FontWeight = FontWeights.Bold }); p.Inlines.Add(new Run(Safe(reason)) { Foreground = Muted }); d.Blocks.Add(p); }
