...
namespace A2ZSysIns
{
    ...
    internal sealed class CpuStressSafetyMonitor
    {
        ...
        public void SetBaseline(double clockMHz) { if (clockMHz >= MinimumUsableClockMHz && clockMHz < 10000) BaselineClockMHz = clockMHz; }
        ...
        public void SetStressBaseline(IEnumerable<double> clocks)
        {
            ...
            BaselineClockMHz = Median(valid); _stressBaselineSet = true; _clocks.Clear();
            foreach (var value in valid) Add(_clocks, value);
            _clockDropCount = 0; _clockSurgeCount = 0;
        }
        ...
        public CpuSafetyDecision Evaluate(CpuSafetyMetrics metrics, bool underStress)
        {
            ...
            if (!metrics.TemperatureC.HasValue || metrics.TemperatureC.Value <= 0 || metrics.TemperatureC.Value >= 150) return AbortNow("CPU temperature telemetry was lost or became invalid.");
            if (metrics.TemperatureC.Value >= HardTemperatureC) return AbortNow("CPU reached the 90 °C hard safety limit.");
            if (underStress && !metrics.AverageCoreClockMHz.HasValue) return AbortNow("CPU clock telemetry was lost during the stress test.");
            if (underStress && (!metrics.CpuLoadPercent.HasValue || metrics.CpuLoadPercent.Value < 0 || metrics.CpuLoadPercent.Value > 100)) return AbortNow("CPU load telemetry was lost or became invalid during the stress test.");
            if (metrics.AverageCoreClockMHz.HasValue && !IsUsableClock(metrics.AverageCoreClockMHz.Value)) return AbortNow("CPU clock telemetry became physically implausible.");
            ...
            if (metrics.AverageCoreClockMHz.HasValue)
            {
                ...
                if (underStress && _stressBaselineSet && metrics.CpuLoadPercent.GetValueOrDefault() >= 70)
                {
                    ...
                    if (_clockDropCount >= RequiredSustainedSamples) return AbortNow("Sustained CPU clock collapse detected under load; possible thermal, power, or stability throttling.");
                    if (_clockSurgeCount >= 2) return AbortNow("Sustained abnormal CPU clock surge detected under load; possible unstable frequency behavior.");
                }
            }
            return new CpuSafetyDecision { Assessment = metrics.TemperatureC.Value >= 85 ? "High temperature — closely monitored" : "Observed", SampleValid = true };
        }
        ...
        private static double Median(IEnumerable<double> values)
        {
            ...
            var middle = sorted.Length / 2; return sorted.Length % 2 == 0 ? (sorted[middle - 1] + sorted[middle]) / 2.0 : sorted[middle];
        }
    }
}